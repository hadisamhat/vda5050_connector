...
bool State::hasActiveOrder() const {
  ...
  int remaining_actions =
      count_if(this->actionStates.begin(), this->actionStates.end(), [](const ActionState& as) {
        return as.actionStatus == interface::ActionStates::INITIALIZING ||
               as.actionStatus == interface::ActionStates::WAITING ||
               as.actionStatus == interface::ActionStates::RUNNING ||
               as.actionStatus == interface::ActionStates::PAUSED;
      });
